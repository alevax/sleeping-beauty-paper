#the first three variables overwrite the normal somaticInteractions function

somaticInteractions = function(oncomatrix_passed, all.tsbs_passed, gene_sum_passed, top = 25, genes = NULL, pvalue = c(0.05, 0.01), returnAll = TRUE,
                               geneOrder = NULL, fontSize = 0.8, showSigSymbols = TRUE,
                               showCounts = FALSE, countStats = 'all', countType = 'all', plot = FALSE,
  ...
  # om = createOncoMatrix(m = maf, g = genes, add_missing = FALSE)
  # all.tsbs = levels(getSampleSummary(x = maf)[,Tumor_Sample_Barcode])
  # mutMat = t(om$numericMatrix)
  
  om = oncomatrix_passed
  all.tsbs = all.tsbs_passed
  mutMat = t(om)
  ...
  if(plot){
    ...
    if(nrow(interactions) >= 5){
      ...
      if(showSigSymbols){
        ...
        points(w, pch=pvSymbols[1], col="white", cex = 1.5*sigSymbolsSize)
      }
      ...
      if(showSigSymbols){
        ...
        points(x = n-nShiftSymbols, y = 0.65*n, pch = pvSymbols[1], cex = 1.5*sigSymbolsSize) # "."
